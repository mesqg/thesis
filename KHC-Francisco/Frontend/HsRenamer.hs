...
instance PrettyPrint RnEnv where
  ppr (RnEnv cls_infos dc_infos tc_infos {-conv_infos-})
    ...
    --, text "rn_env_conv_info"  <+> colon <+> ppr conv_infos
    ]
  needsParens _ = False
...
{-- | Get all renamed conversion names from the state
getCnvInfoRnM :: RnM (AssocList PsIConv HsIConvInfo)
getCnvInfoRnM = rn_env_conv_info <$> get -}
...
{-- | Add a renamed conversion name to the state
addCnvInfoRnM :: PsIConv -> HsIConvInfo -> RnM ()
addCnvInfoRnM cnv info = modify $ \s ->
  s { rn_env_conv_info = extendAssocList cnv info (rn_env_conv_info s) }
-}
...
rnClass cls = do
  ...
  case lookupInAssocList cls cls_infos of
    ...
    Nothing       -> throwErrorRnM (text "Class name" <+> ppr cls <+> text "unbound")
-- | May be dumb
{-- | Rename a conversion name
rnConv :: PsIConv -> RnM RnIConv
rnConv conv = do
  cnv_infos <- getCnvInfoRnM
  case lookupInAssocList conv cnv_infos of
    Just cnv_info -> return (cnv_name cnv_info)
    Nothing       -> throwErrorRnM (text "Conversion name" <+> text "unbound")-}
...
rnProgram (PgmImpl impl_decl pgm) = do
  rn_impl_decl <- rnIConvDecl impl_decl
  (rn_pgm, rn_ctx) <- rnProgram pgm
  return (PgmImpl rn_impl_decl rn_pgm, rn_ctx)
