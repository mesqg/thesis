...
-}  
solveY :: YCs -> ImplicitTheory -> TcM FcTmSubst
solveY SN it = return mempty
solveY (xs :> x@(YCt j _)) it = do
  {-
  (expr, ty_subst) <- solve_one_iconv x it
  rest <- solveY (substInYCs ty_subst xs) it
  return (j |-> expr <> rest)
  -}
  (subst,ty_subst) <- entailTcM2 [] it (singletonSnocList x)
  rest <- solveY (substInYCs ty_subst xs) it
  return (subst <> rest)
 {-  
  where solve_one_iconv :: YCt -> ImplicitTheory -> TcM (FcTerm,HsTySubst)
        ...
        solve_one_iconv _ SN = do return (FcDummyTerm,mempty)
-- TODO duplicated code! -}
-- | Completely entail a set of constraints. Fail if not possible
entailTcM2 :: [RnTyVar] ->  ImplicitTheory -> YCs -> TcM (FcTmSubst,HsTySubst)
entailTcM2 untch theory ctrs = runSolverFirstM (go ctrs)
  where
    go (cs :> c) = do
