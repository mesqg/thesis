...
\section{Typing}
\label{3typing}
A HM type is well-formed ``$\tyEnv \vdash_{ty} \typeS$'' if all its type variables are in the typing environment as the type constructors are in scope, as shown in \fref{tywf}. The typing rules (shown in \fref{hmty}) are very similar to System F's, except that:
\begin{itemize}
...
\begin{flushleft}
  \item \ruleform{ \tyEnv \vdash_{ty}\typeS }
\end{flushleft}
\[
...
The first part of algorithm W, concerned with the equality constraint generation is shown in \fref{hmtyinf}. The highlighted part is important for the translation of HM terms to System F (Section \ref{3elab}), but can be safely ignored for now. The relation ``$\tyEnv \vdash_{tm} \term : \type ;\;E$'', which should be read as ``under the environment $\tyEnv$, gets assigned type $\type$ assuming the equality constraints $E$ are satisfied'', reflects the behavior of the first part of the algorithm.
...
\begin{flushleft}  
  ...
  \namedRuleform{ \tyEnv \vdash_{tm} \term : \typeS \highlight{;\term^F}}
...
\section{Elaboration}
\label{3elab}
The elaboration of a HM to System F is presented highlighted in \fref{hmtyinf}. Notice the rule \textsc{T-Var}: because System F has explicit type abstraction (and application), a variable that is bound to a polytype is applied type variables to that these are later substituted by the appropriate types.
