...
instance Uniquable a => Uniquable (IConv a) where
  getUnique = getUnique . unIConv

instance Symable a => Symable (IConv a) where
  symOf = symOf . unIConv

instance Named a => Named (IConv a) where
  nameOf = nameOf . unIConv  

type PsIConv = IConv Sym
type RnIConv = IConv Name

data HsIConvInfo = HsICInfo {cnv_name :: RnIConv}
-}
-- * Class Names
-- ------------------------------------------------------------------------------
...
-- | Take a qualified type apart
destructQualConvTy :: QualConvTy a -> ([MonoConvTy a], MonoConvTy a)
destructQualConvTy (QCTS    ty) = ([], ty)
destructQualConvTy (QCTC ct ty) = (ct:cs, ty')
  where (cs, ty') = destructQualConvTy ty

-- | Inverse of destructPolyConvTy: create a polytype from parts
constructPolyConvTy :: ([HsTyVarWithKind a], [MonoConvTy a], MonoConvTy a) -> PolyConvTy a
constructPolyConvTy (as, cs, ty) = foldr PCTC (PCTS (constructQualConvTy (cs,ty))) as
...
-- | Parsed/renamed implicit conversion declarations
type PsIConvDecl = IConvDecl Sym
type RnIConvDecl = IConvDecl Name

-- * Additional Syntax For Type Inference And Elaboration
-- ------------------------------------------------------------------------------

-- | All constraints (equality and class) --UPDATED and IConvs
type Cs = (EqCs, YCs, RnClsCs)
...
-- * Pretty Printing
-- ------------------------------------------------------------------------------
-- | Pretty print for implicit conv info
{-instance PrettyPrint HsIConvInfo where
  ppr (HsICInfo name)
    = braces $ vcat $ punctuate comma
    $ [
        text "implConv info" <+> colon <+> ppr name
      ]
  needsParens _ = False -}
...
-- | Pretty print class names
{-instance Symable a => PrettyPrint (IConv a) where
  ppr           = ppr . symOf
  needsParens _ = False
  -}
...
isIConvtr :: Ctr a -> Bool
isIConvtr (Ctr [] [] _) = False
isIConvtr (Ctr [] _ _)  = True
isIConvtr _other        = False
...
instance (Symable a, PrettyPrint a) => PrettyPrint (Program a) where
  ...
  ppr (PgmImpl icdcl pgm) = ppr icdcl $$ ppr pgm
  needsParens _ = False
