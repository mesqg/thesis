...
reachable init it = aux [init] [] it
   where 
         ...
         aux acc q@(at:rest) (xs:>x) = case onestep acc at x of 
         ...
         onestep acc at (UC (MCT a b) exp)
          | Right ty_subst <- unify [] [at :~: a] =
              if (substInMonoTy ty_subst b) `elem` acc then Nothing else
...
step1 all@(rest:>conv@(YCt _ (MCT a (TyVar b))))
 | one_incomming (TyVar b) all = substInYCs ( b |-> a) rest
 | otherwise      = (step1 rest):>conv
 where one_incomming b SN = True
       one_incomming b (xs:>x@(YCt _ (MCT _ c))) = if c == b then False else one_incomming b xs
step1 SN = SN
